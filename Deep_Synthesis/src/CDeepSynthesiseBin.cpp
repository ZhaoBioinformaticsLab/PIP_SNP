...
void CDeepSynthesis::Load_One_Bin(LDBin_Object *&Read_Bin)
{
    if(Processing_Bin_Buffer.size()>3)
    {
       cerr ("Processing Bin Bufffer is full! There are logical error");
    }
...
void CDeepSynthesis::Process_ReadBin(LDBin_Object *&Read_Bin)
{
    if(Processing_Bin_Buffer.size()!=3)
    {
        cerr("The processing bin buffer is not full(3), logical error!");
    }
    else
    {
        ...
        switch(LDBin_Method_Corr)
        {
           case Pairwise_PCC:  // 0:
               R2_01 = Calculate_Pairwise_PCC((*m_Bin0).LDBinData, (*m_Bin1).LDBinData, LDBin_Individual_Num);
               R2_02 = Calculate_Pairwise_PCC((*m_Bin0).LDBinData, (*m_Bin2).LDBinData, LDBin_Individual_Num);
               break;
           case Pairwise_LD:  //1:
               R2_01 = Calculate_Pairwise_LD((*m_Bin0).LDBinData, (*m_Bin1).LDBinData, LDBin_Individual_Num);
               R2_02 = Calculate_Pairwise_LD((*m_Bin0).LDBinData, (*m_Bin2).LDBinData, LDBin_Individual_Num);
               break;
           default:
               R2_01 = Calculate_Pairwise_PCC((*m_Bin0).LDBinData, (*m_Bin1).LDBinData, LDBin_Individual_Num);
               R2_02 = Calculate_Pairwise_PCC((*m_Bin0).LDBinData, (*m_Bin2).LDBinData, LDBin_Individual_Num);
        ...
        if(R2_01 >=LDBin_R2Th)
        {
           ...
           Processing_Bin_Buffer.pop_back(); // Remove the first
        }
        else
        {
            if(R2_02>=LDBin_R2Th)
            {
                ...
                Processing_Bin_Buffer.erase(Processing_Bin_Buffer.begin()+1);
            }
            else
            {
...
void CDeepSynthesiseBin::Synthesis_Combine_Process()
{
    ...
    if(Uncontinuous_Bin_Buffer.size()>0)
    {
        while(!Uncontinuous_Bin_Buffer.empty())
        {
            ...
            UB_Size=Uncontinuous_Bin_Buffer.size();
            if(UB_Size==1) // only 1 bin
            {
                DS_Processed_Bin_Buffer.push_back(Uncon_Bin1);
                Uncontinuous_Bin_Buffer.pop_front(); // remove the only one bin
            }
            else // 2 more bins
            {
                float R2_Neighbor2;
                ...
                Uncontinuous_Bin_Buffer.pop_front();
                LDBin_Object *Uncon_Bin2 = Uncontinuous_Bin_Buffer.front();
                pdata_bin1=(*Uncon_Bin1).LDBinData;
                pdata_bin2=(*Uncon_Bin2).LDBinData;

                R2_Neighbor2 = Calculate_r2_one_bin_to_another_bin(pdata_bin1, fpdata_bin2, LDBin_Individual_Num, LDBin_Corr_Method);
                ...
                if(R2_Neighbor2>LDBin_R2Th) //merge the two neighnor bins into one due to their high correlation
                {
                  ...
                  DS_Processed_Bin_Buffer.push_back(Uncon_Bin1);
                  Uncontinuous_Bin_Buffer.pop_front(); // remove the second bin
              }
